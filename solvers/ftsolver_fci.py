...
def microIteration(dmet,mu0,R,h_emb,V_emb,targetN, gtol, maxiter = 20):
    #if dmet.T < 1e-3:
    #    return microIteration_fitmu(dmet,mu0,R,h_emb,V_emb,targetN, gtol, maxiter)
    print "Solving impurity problem with chemical potential = ", dmet.grandmu
    sys.stdout.flush()
    Pn, P2n, Enewn = ftmodules.ftimpsolver(dmet,h_emb,V_emb,h_emb.shape[0]/2,dmet.actElCount)
...
###############################################################################     
def microIteration_fitmu(dmet,mu0,R,h_emb,V_emb,targetN, gtol, maxiter = 20):
