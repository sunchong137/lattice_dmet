...
def hf_hubbard_calc(Nelec,Hcore,g,Porig=None,S=None,itrmax=0, needEnergy = False, assertcheck = False, doDIIS = False, doShift = False, hfoscitr = 0, gap = None):
    
    #simplified subroutine to perform calculation for Hubbard model with no SOC ONLY useful for RHF
    #Diagonalize hopping-hamiltonian
    evals,orbs = diagonalize(Hcore)
    if abs(evals[Nelec-1]-evals[Nelec]) < 1e-16:
        print "Degeneracy of Mean-field eigenvalues occurred! Using finite T perturbation!"
        beta = 100
        def fermi(mu):
            return 1./(1.+np.exp((evals-mu)*beta)) 
        def costf(mu):
            return abs(Nelec-np.sum(fermi(mu)))
        mu0 = 0.0
        mu = minimize(costf, mu0, method='Powell',options = {'maxiter':10, 'disp':False}).x
        nocc = fermi(mu)
        #print nocc
        #exit()
        P = np.dot(orbs, np.dot(np.diag(nocc), orbs.conj().T))
    #Form the 1RDM
    else:
    ...
    if(gap is not None and lgap/gap<1.0e-2): #Probably an issue if local gap is smaller than gap by two orders of magnitude
        print "LG: ",order(lgap)," GO: ",order(gap)
        P += np.eye(P.shape[0])*(np.exp(abs(order(lgap)-order(gap)))-1)
    '''
